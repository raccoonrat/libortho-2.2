...
class OrthoLinear(nn.Module):
    def __init__(self, original_layer, config: Optional[OrthoConfig] = None):
        ...
        # 3. 策略执行 (Enhanced Deep-Focal Logic)
        ...
        if self.config.noise_mode == 'deterministic':
            ...
            # {4, 5, 6, 7}. Min=4. 
            # PROFESSOR'S UPDATE: Raised protection threshold to 4.2
            # 保护 Body (0-3.5) 和 Middle Class (3.5-4.2)
            # 只攻击真正的 Deep Outliers (> 4.2)
            # 这允许我们在使用强噪声的同时，不误伤中产阶级
            is_deep = w_scaled.abs() > 4.2
            should_attack = is_outlier & is_deep
...
def replace_linear_layers(model, target_modules=["down_proj", "o_proj"], ratio=0.05):
    ...
    print(f"[LibOrtho] Initializing Physics-Constrained Auto-Tuner (Deep-Focal v2)...")
